...
const MobileSidebar: React.FC = () => {
  ...
  return (
    <nav className="md:hidden bg-Secondary dark:bg-DarkSecondary block bottom-0 w-full">
      <ul
        className={`z-50 fixed transition-transform w-full flex justify-between items-center bottom-0 left-0 bg-Primary dark:bg-DarkPrimary text-2xl text-gray-500 dark:text-gray-400 pb-2 px-3
          ...
          links.map(link => (
            <li key={link.name}>
              <Link
                ...
                {<link.icon />}
                <span className="text-[.95rem] -mt-2 tracking-wide">{link.name}</span>
              </Link>
            </li>
          ))
        }
